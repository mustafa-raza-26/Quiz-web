...
// Micro-interaction: Card Click Animation
...
document.querySelectorAll('.glass-card[data-category]').forEach(card => {
    card.addEventListener('click', () => {
        ...
        window.location.href = `test.html?category=${encodeURIComponent(category)}`;
    });
});

// Simple mouse parallax for background feel
document.addEventListener('mousemove', (e) => {
    const x = (window.innerWidth - e.pageX * 2) / 100;
    const y = (window.innerHeight - e.pageY * 2) / 100;
    const animationElement = document.querySelector('threejs-scene');
    if (animationElement) {
